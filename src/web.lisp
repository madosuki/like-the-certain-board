          ...
          ((string= mode "delete")
           (if (and (not (null key)) (check-exist-row (parse-integer key)))
               ...
               (prgon
                ...
                "invalid params")))
          ((string= mode "log")
           (let* ((thread-data (get-a-thread (parse-integer key)))
                  (title (getf thread-data :title)))
                       ...
                       (if (to-kakolog key filepath)
                           (let ((result nil))
                             (handler-case  (copy-file filepath (format nil "~A/~A.dat" *kakolog-dat-path* key))
                               (error (e)
                                 (format t "~%~%~A~%~%" e)
                                 (delete-file (format nil "~A/~A.html" *kakolog-html-path* key))
                             ...
                             (unless result
                               (if (delete-file filepath)
                                   (progn
                                     ...
                                     (set-response-status 302)
                                     (next-route))
                                   (progn (setq result "failed delete file")
                                          (delete-file (format nil "~A/~A.html" *kakolog-html-path* key)))))
                             result)
                           "failed convert html")
                       "not exists thread"))
