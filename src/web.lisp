...
(defroute ("/:board-name/login" :method :GET) (&key board-name)
  (let ((ipaddr (caveman2:request-remote-addr caveman2:*request*))
        (board-data (get-a-board-name-from-name board-name))
        (user-agent (gethash "user-agent" (caveman2:request-headers caveman2:*request*))))
    (if (and (equal ipaddr *admin-ipaddr*) board-data (null (detect-monazilla user-agent)))
          ...
          ((equal mode "login")
           ...
           ((equal mode "create")
            (if (string= ipaddr *admin-ipaddr*)
                (let ((create-result (create-user (getf board-data :id) user-name password date is-admin cap-text)))
                  (set-response-status 401)
